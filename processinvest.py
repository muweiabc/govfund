...
def filter_company(df):
    ...
    filtered_companies[1:].to_excel('invest.xlsx', sheet_name='去重公司列表')
...
def add_treatment_column():
    ...
    try:
        ...
        print("5. 保存更新后的文件...")
        output_filename = 'invest.xlsx'
        invest_df.to_excel(output_filename, sheet_name='有专利公司首次投资', index=False)
...
def extract_first_investment_for_patent_companies():
    ...
    在invest_with_treatment中搜索每个公司获得的最早的一笔投资，
    把该行输出到invest_with_treatment的新sheet里
    """
    try:
        ...
        # 2. 读取投资数据
        print("2. 读取投资数据...")
        invest_df = pd.read_excel('invest_with_treatment.xlsx')
