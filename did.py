...
def perform_did_regression_with_gdp():
    """
    根据regress_data_with_gdp数据做DID回归
    被解释变量是公司某年的ln(专利数+1)
    treatment是是否接受政府引导基金投资
    post变量在投资后3年取1，投资前3年取0
    控制变量包括公司所在省份当年ln(GDP+1)
    """
    try:
        print("=== 执行带GDP控制变量的DID回归分析 ===")
        ...
        try:
            ...
            X = panel_df[['treatment', 'post', 'treatment_post', 'ln_gdp']]
            ...
            # 保存面板数据
            panel_filename = 'did_panel_data.xlsx'
            with pd.ExcelWriter(panel_filename, engine='openpyxl') as writer:
                ...
                results_summary = pd.DataFrame({
                    '变量': ['Treatment', 'Post', 'Treatment×Post', 'ln(GDP+1)'],
                    '系数': [results.params['treatment'], results.params['post'], 
                           results.params['treatment_post'], results.params['ln_gdp']],
                    '标准误': [results.std_errors['treatment'], results.std_errors['post'], 
                             results.std_errors['treatment_post'], results.std_errors['ln_gdp']],
                    't值': [results.tstats['treatment'], results.tstats['post'], 
                           results.tstats['treatment_post'], results.tstats['ln_gdp']],
                    'p值': [results.pvalues['treatment'], results.pvalues['post'], 
        ...
        print(f"执行带GDP控制变量的DID回归时出现错误: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def perform_did_regression_with_year_dummies():
    ...
    try:
        ...
        # 创建年份虚拟变量（排除基准年份，避免完全共线性）
        base_year = min(all_years)  # 使用最早年份作为基准
        ...
        try:
            ...
            # 准备回归变量（包含GDP控制变量和年份虚拟变量）
            control_vars = ['treatment', 'post', 'treatment_post', 'ln_gdp'] + year_dummy_cols
            X = panel_df[control_vars]
            y = panel_df['ln_patent_plus_1']
            
            print(f"   - 回归变量数量: {len(control_vars)}")
            print(f"   - 控制变量: {len(year_dummy_cols)} 个年份虚拟变量 + GDP")
            ...
            # 显示年份虚拟变量的显著性
            print(f"\n10. 年份虚拟变量显著性:")
            significant_year_dummies = []
            for col in year_dummy_cols:
                if col in results.params.index:
                    p_value = results.pvalues[col]
                    if p_value < 0.05:
                        significant_year_dummies.append((col, results.params[col], p_value))
            
            if significant_year_dummies:
                print(f"   - 显著的年份虚拟变量 (p<0.05): {len(significant_year_dummies)} 个")
                for dummy, coef, p_val in significant_year_dummies[:10]:  # 只显示前10个
                    year_name = dummy.replace('year_', '')
                    print(f"     {year_name}年: 系数={coef:.4f}, p值={p_val:.4f}")
            else:
                print("   - 没有显著的年份虚拟变量")
            ...
            with pd.ExcelWriter(panel_filename, engine='openpyxl') as writer:
                ...
                results_summary = pd.DataFrame({
                    '变量': ['Treatment', 'Post', 'Treatment×Post', 'ln(GDP+1)'] + 
                           [f'年份{col.replace("year_", "")}年' for col in year_dummy_cols],
                    '系数': [results.params['treatment'], results.params['post'], 
                           results.params['treatment_post'], results.params['ln_gdp']] +
                           [results.params[col] for col in year_dummy_cols],
                    '标准误': [results.std_errors['treatment'], results.std_errors['post'], 
                             results.std_errors['treatment_post'], results.std_errors['ln_gdp']] +
                             [results.std_errors[col] for col in year_dummy_cols],
                    't值': [results.tstats['treatment'], results.tstats['post'], 
                           results.tstats['treatment_post'], results.tstats['ln_gdp']] +
                           [results.tstats[col] for col in year_dummy_cols],
                    'p值': [results.pvalues['treatment'], results.pvalues['post'], 
                           results.pvalues['treatment_post'], results.pvalues['ln_gdp']] +
                           [results.pvalues[col] for col in year_dummy_cols]
                })
                results_summary.to_excel(writer, sheet_name='回归结果', index=False)
                ...
                # 保存年份虚拟变量统计
                year_dummy_stats = panel_df[year_dummy_cols].describe()
                year_dummy_stats.to_excel(writer, sheet_name='年份虚拟变量统计')
            ...
            return {
                ...
                'did_p_value': results.pvalues['treatment_post'],
                'gdp_effect': results.params['ln_gdp'],
                'year_dummy_count': len(year_dummy_cols),
                'significant_year_dummies': len(significant_year_dummies),
                'panel_file': panel_filename
            }
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print(f"年份虚拟变量数量: {result['year_dummy_count']}")
        print(f"显著的年份虚拟变量: {result['significant_year_dummies']}")
